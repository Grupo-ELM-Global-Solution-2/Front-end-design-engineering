...
export default function TrilhaPersonalizadaCard({ trilha, onVerTrilha, onDeleteTrilha }: TrilhaPersonalizadaCardProps) {
    return (
        <div className="card-white">
            <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex items-center gap-2">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                        trilha.experiencia === 'iniciante' ? 'bg-green-100 text-green-800' :
                        trilha.experiencia === 'intermediario' ? 'bg-yellow-100 text-yellow-800' :
                        'bg-red-100 text-red-800'
                    }`}>
                        {trilha.experiencia}
                    </span>
                    <button onClick={() => onDeleteTrilha(trilha)} className="text-red-500 hover:text-red-700 text-lg" title="Excluir trilha">🗑️</button>
                </div>
            </div>
            <p className="text-sm text-gray-600 mb-4">
                Tempo disponível: {trilha.disponibilidade} horas/semana
            </p>
            ...
            <button onClick={() => onVerTrilha(trilha)} className="btn-primary w-full">Ver Trilha Completa</button>
        </div>
    );
}
