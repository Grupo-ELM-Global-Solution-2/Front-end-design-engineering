...
export default function Card({ icon, iconClassName, title, description, className, variant = 'default' }: CardProps) {
  ...
  return (
    <div className={`text-center ${isBenefit ? 'p-6' : isStep ? '' : 'p-8'} bg-white rounded-lg ${isBenefit ? 'border border-gray-200' : ''} ${className || ''}`}>
      {icon && (
      ...
      <h3 className={`${isBenefit ? 'text-lg' : 'text-xl'} font-semibold text-gray-900 ${isBenefit ? 'mb-2' : 'mb-3'}`}>{title}</h3>
      <p className={`text-gray-600 ${isBenefit ? 'text-sm' : ''}`}>{description}</p>
    </div>
  );
}
